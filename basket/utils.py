...
# class CustomBasketMessageGenerator(BasketMessageGenerator):
#     """
#     customised basket message generator
#     """
...
def user_coupon_code_transaction(order):
    import requests
    import json
    ...
    try:
        if settings.DEBUG:
            request_url = "http://localhost:7000/api/v1/transaction-create"
        else:
            request_url = "http://coupon.instafitness.in/api/v1/transaction-create"

        if order.payment_done == True:
            payment_status = "SUCCESS"
       
        data = {
                "code":order.basket.vouchers.first().code,
                "order_number": int(order.number),                
                "total_amount":float(order.basket_total_before_discounts_incl_tax),
                "transaction_amount": float(order.total_incl_tax), #-float(order.basket.vouchers.first().total_discount),
...
def lead_create(lead):
    ...
    try:
        if settings.DEBUG:
            request_url = "http://localhost:8000/leads/api/v1/lead_create"
        else:
            request_url = "http://staging.instafitness.in/leads/api/v1/lead_create"

        data = {
                "name":lead['first_name'] + " "+ lead['last_name'],
                "mobile_number": "+91" + lead['mobile_number'],                
                "source":"IE"
            }
